...
type (
	ICommentCache interface {
		...
		SetPostComment(ctx context.Context, postID uuid.UUID, commentIds []uuid.UUID, paging *response.PagingResponse)
		GetPostComment(ctx context.Context, postID uuid.UUID, limit, page int) ([]uuid.UUID, *response.PagingResponse)
