...
func (c *cUserAuth) VerifyEmail(ctx *gin.Context) {
	...
	err := services.UserAuth().VerifyEmail(ctx, verifyEmailRequest.Email)
	if err != nil {
		ctx.Error(err)
		return
	}
...
func (c *cUserAuth) Register(ctx *gin.Context) {
	...
	_, err = services.UserAuth().Register(ctx, registerCommand)
	if err != nil {
		ctx.Error(err)
		return
	}
...
func (c *cUserAuth) ChangePassword(ctx *gin.Context) {
	...
	err = services.UserAuth().ChangePassword(ctx, changePasswordCommand)
	if err != nil {
		ctx.Error(err)
		return
	}
...
func (c *cUserAuth) ForgotUserPassword(ctx *gin.Context) {
	...
	err = services.UserAuth().ForgotUserPassword(ctx, forgotUserPasswordCommand)
	if err != nil {
		ctx.Error(err)
		return
	}
