...
type cRevenueAdmin struct {
	revenueService services.IRevenue
}

func NewRevenueAdminController(
	revenueService services.IRevenue,
) *cRevenueAdmin {
...
func (c *cRevenueAdmin) GetMonthlyRevenue(ctx *gin.Context) {
	...
	result, err := c.revenueService.GetMonthlyRevenue(ctx, monthlyRevenueQuery)
	if err != nil {
		ctx.Error(err)
		return
	}
...
func (c *cRevenueAdmin) GetSystemStats(ctx *gin.Context) {
	...
	result, err := c.revenueService.GetSystemStats(ctx, systemStatsQuery)
	if err != nil {
		ctx.Error(err)
		return
	}
