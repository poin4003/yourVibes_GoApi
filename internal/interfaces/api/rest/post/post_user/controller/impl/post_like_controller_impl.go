...
type cPostLike struct {
	likePostService services.IPostLike
}
...
func (c *cPostLike) LikePost(ctx *gin.Context) {
	...
	result, err := c.likePostService.LikePost(ctx, likePostCommand)
	if err != nil {
		ctx.Error(err)
		return
	}
...
func (c *cPostLike) GetUserLikePost(ctx *gin.Context) {
	// 1. Get query
	...
	result, err := c.likePostService.GetUsersOnLikes(ctx, getPostLikeQuery)
	if err != nil {
		ctx.Error(err)
		return
	}
