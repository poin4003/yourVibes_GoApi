...
func (c *cMessage) CreateMessage(ctx *gin.Context) {
	...
	err = c.messageService.CreateMessage(ctx, createMessageCommand)
	if err != nil {
		ctx.Error(err)
		return
	}
...
func (c *cMessage) GetMessageById(ctx *gin.Context) {
	...
	result, err := c.messageService.GetMessageById(ctx, messageId)
	if err != nil {
		ctx.Error(err)
		return
	}
...
func (c *cMessage) GetMessagesByConversationId(ctx *gin.Context) {
	...
	result, err := c.messageService.GetMessagesByConversationId(ctx, getMessagesByConversationIdQuery)
	if err != nil {
		ctx.Error(err)
		return
	}
...
func (c *cMessage) DeleteMessageById(ctx *gin.Context) {
	messageIdStr := ctx.Param("messageId")
	messageId, err := uuid.Parse(messageIdStr)
	if err != nil {
		ctx.Error(pkgResponse.NewValidateError(err.Error()))
