...
func (c *cUserInfo) GetInfoByUserId(ctx *gin.Context) {
	...
	result, err := c.userInfoService.GetInfoByUserId(ctx, getOneUserQuery)
	if err != nil {
		ctx.Error(err)
		return
	}
...
func (c *cUserInfo) GetManyUsers(ctx *gin.Context) {
	...
	result, err := c.userInfoService.GetManyUsers(ctx, getManyUserQuery)
	if err != nil {
		ctx.Error(err)
		return
	}
...
// @Security ApiKeyAuth
// @Router       /users/ [patch]
func (c *cUserInfo) UpdateUser(ctx *gin.Context) {
	...
	result, err := c.userInfoService.UpdateUser(ctx, updateUserCommand)
	if err != nil {
		ctx.Error(err)
		return
	}
