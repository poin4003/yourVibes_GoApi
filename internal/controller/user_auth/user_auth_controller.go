...
func (c *cUserAuth) VerifyEmail(ctx *gin.Context) {
	...
	if err := ctx.ShouldBindJSON(&input); err != nil {
		response.ErrorResponse(ctx, response.ErrCodeValidateParamEmail)
		return
	}
...
func (c *cUserAuth) Register(ctx *gin.Context) {
	...
	if err := ctx.ShouldBindJSON(&registerInput); err != nil {
		response.ErrorResponse(ctx, response.ErrCodeValidateParamRegister)
		return
	}
...
func (c *cUserAuth) Login(ctx *gin.Context) {
	...
	if err := ctx.ShouldBindJSON(&loginInput); err != nil {
		response.ErrorResponse(ctx, response.ErrCodeValidateParamLogin)
		return
	}

	accessToken, user, err := services.UserAuth().Login(ctx, &loginInput)
	if err != nil {
		response.ErrorResponse(ctx, response.ErrCodeLoginFailed)
		return
	}
